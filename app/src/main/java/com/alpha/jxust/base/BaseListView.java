...
import com.alpha.jxust.ui.xlist.XListView;
    ...
    public void loadDatas(final boolean loadMore) {
        ...
        new StatusNetAsyncTask(getContext()) {
            ...
            protected void onPost(Exception e) {
                if (e != null) {
                    e.printStackTrace();
                    StatusUtil.toast(getContext(), e.getMessage());
                } else {
                    if (loadMore == false) {
                        stopRefresh();
                        ...
                        if (datas.size() < ONE_PAGE_SIZE) {
                            if (isToastIfEmpty()) {
                                if (datas.size() == 0) {
                                    StatusUtil.toast(getContext(), R.string.listEmptyHint);
                                }
                            }
                        ...
                        if (datas.size() == 0) {
                            StatusUtil.toast(getContext(), R.string.noMore);
                        }
                    }
                }
            }
